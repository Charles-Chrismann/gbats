...
export class ARMCore {
  ...
  WARN: (arg0: string) => void
		...
		if (newMode != this.MODE_USER || newMode != this.MODE_SYSTEM) {
			...
			if (newBank != oldBank) {
				...
				if (newMode == this.MODE_FIQ || this.mode == this.MODE_FIQ) {
					var oldFiqBank = (oldBank == this.BANK_FIQ) as any + 0;
					var newFiqBank = (newBank == this.BANK_FIQ) as any + 0;
	...
	packCPSR() {
		return (
			this.mode |
			(((!!this.execMode) as unknown as number) << 5) |
			(((!!this.cpsrF) as unknown as number) << 6) |
			(((!!this.cpsrI) as unknown as number) << 7) |
			(((!!this.cpsrN) as unknown as number) << 31) |
			(((!!this.cpsrZ) as unknown as number) << 30) |
			(((!!this.cpsrC) as unknown as number) << 29) |
			(((!!this.cpsrV) as unknown as number) << 28)
		);
	}
	unpackCPSR(spsr) {
		...
		switch (shiftType) {
			...
			case 0x00000060:
				...
				if (immediate) {
					...
					shiftOp = function () {
						cpu.shifterOperand =
            (((!!cpu.cpsrC) as unknown as number) << 31) | (gprs[rm] >>> 1);
	...
	compileArm(instruction) {
		var op: any = this.badOp(instruction);
				...
				case 0x06000000:
					...
					if (i) {
						...
						if (shiftType || shiftImmediate) {
							var shiftOp: any = this.barrelShiftImmediate(
								shiftType,
								shiftImmediate,
								rm
							);
	...
	compileThumb(instruction) {
		var op: any = this.badOp(instruction & 0xffff);
		var cpu = this;
		var gprs = this.gprs;
