...
export class GameBoyAdvanceSoftwareRenderer {
  ...
  removeBacking() {
	...
	writeBlendControl(value) {
		this.target1[0] = (!!(value & 0x0001)) as unknown as number * this.TARGET1_MASK;
		this.target1[1] = (!!(value & 0x0002)) as unknown as number * this.TARGET1_MASK;
		this.target1[2] = (!!(value & 0x0004)) as unknown as number * this.TARGET1_MASK;
		this.target1[3] = (!!(value & 0x0008)) as unknown as number * this.TARGET1_MASK;
		this.target1[4] = (!!(value & 0x0010)) as unknown as number * this.TARGET1_MASK;
		this.target1[5] = (!!(value & 0x0020)) as unknown as number * this.TARGET1_MASK;
		this.target2[0] = (!!(value & 0x0100)) as unknown as number * this.TARGET2_MASK;
		this.target2[1] = (!!(value & 0x0200)) as unknown as number * this.TARGET2_MASK;
		this.target2[2] = (!!(value & 0x0400)) as unknown as number * this.TARGET2_MASK;
		this.target2[3] = (!!(value & 0x0800)) as unknown as number * this.TARGET2_MASK;
		this.target2[4] = (!!(value & 0x1000)) as unknown as number * this.TARGET2_MASK;
		this.target2[5] = (!!(value & 0x2000)) as unknown as number * this.TARGET2_MASK;
	...
	static pushPixel(layer, map, video, row, x, offset, backing, mask, raw) {
		...
		var highPriority: any = (mask & video.PRIORITY_MASK) < (oldStencil & video.PRIORITY_MASK);
