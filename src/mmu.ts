...
export class GameBoyAdvanceMMU {
  ...
  DMA_REGISTER: number[]
	...
	waitMul(rs) {
		if (rs & (0xffffff00 == 0xffffff00) as unknown as number || !(rs & 0xffffff00)) {
			this.cpu.cycles += 1;
		} else if (rs & (0xffff0000 == 0xffff0000) as unknown as number || !(rs & 0xffff0000)) {
			this.cpu.cycles += 2;
		} else if (rs & (0xff000000 == 0xff000000) as unknown as number || !(rs & 0xff000000)) {
			this.cpu.cycles += 3;
		} else {
			this.cpu.cycles += 4;
		}
