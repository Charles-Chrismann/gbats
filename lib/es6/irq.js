...
exports.GameBoyAdvanceInterruptHandler = void 0;
var mmu_1 = require("./mmu");
var GameBoyAdvanceInterruptHandler = /** @class */ (function () {
        ...
        if (this.timersEnabled) {
            ...
            if (timer.enable) {
                if (this.cpu.cycles >= timer.nextEvent) {
                    ...
                    if (this.audio.enabled) {
                        if (this.audio.enableChannelA &&
                            !this.audio.soundTimerA &&
                            this.audio.dmaA >= 0) {
                            this.audio.sampleFifoA();
                        }
                        if (this.audio.enableChannelB &&
                    ...
                    timer = this.timers[1];
                    if (timer.countUp) {
                        if (++this.io.registers[this.io.TM1CNT_LO >> 1] ==
                            0x10000) {
            ...
            if (timer.enable) {
                if (this.cpu.cycles >= timer.nextEvent) {
                    ...
                    timer = this.timers[2];
                    if (timer.countUp) {
                        if (++this.io.registers[this.io.TM2CNT_LO >> 1] ==
                            0x10000) {
                            timer.nextEvent = this.cpu.cycles;
                        }
                    }
            ...
            if (timer.enable) {
                if (this.cpu.cycles >= timer.nextEvent) {
                    ...
                    timer = this.timers[3];
                    if (timer.countUp) {
                        if (++this.io.registers[this.io.TM3CNT_LO >> 1] ==
                            0x10000) {
                            timer.nextEvent = this.cpu.cycles;
                        }
                    }
    ...
    GameBoyAdvanceInterruptHandler.prototype.swi = function (opcode) {
        ...
        switch (opcode) {
            ...
            case 0x01:
                ...
                if (regions & 0x01) {
                    this.core.mmu.memory[this.core.mmu.REGION_WORKING_RAM] = new mmu_1.MemoryBlock(this.core.mmu.SIZE_WORKING_RAM, 9);
                }
                if (regions & 0x02) {
    ...
    GameBoyAdvanceInterruptHandler.prototype.timerWriteControl = function (timer, control) {
        ...
        else if (currentTimer.prescaleBits != oldPrescale &&
...
exports.GameBoyAdvanceInterruptHandler = GameBoyAdvanceInterruptHandler;
